      ...
      <div class="rounded-2xl bg-white p-8 shadow-xl dark:bg-dark-800">
        ...
        <div v-if="currentStep === 0" class="space-y-6">
          ...
          <template v-if="formData.database.driver === 'mysql'">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div><label class="input-label">{{ t('setup.database.databaseName') }}</label><input v-model="formData.database.dbname" type="text" class="input" placeholder="ikik_api" /></div>
              <div><label class="input-label">{{ t('setup.database.sslMode') }}</label><Select v-model="formData.database.sslmode" :options="[{ value: 'disable', label: t('setup.database.ssl.disable') }, { value: 'require', label: t('setup.database.ssl.require') }, { value: 'verify-ca', label: t('setup.database.ssl.verifyCa') }, { value: 'verify-full', label: t('setup.database.ssl.verifyFull') }]" /></div>
            </div>
          </template>
          <div v-else>
            <label class="input-label">{{ t('setup.database.path') }}</label>
            <input v-model="formData.database.path" type="text" class="input" placeholder="ikik-api.db" />
            <p class="mt-2 text-xs text-gray-500 dark:text-dark-400">{{ t('setup.database.sqliteHint') }}</p>
          </div>
          ...
          <div class="space-y-4">
            <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
              ...
              <p class="text-gray-900 dark:text-white">
                {{ formData.database.driver === 'sqlite'
                  ? `${t('setup.database.sqlite')}: ${formData.database.path}`
