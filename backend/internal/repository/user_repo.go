...
func (r *userRepository) GetLatestUsedAtByUserIDs(ctx context.Context, userIDs []int64) (map[int64]*time.Time, error) {
	...
	for rows.Next() {
		...
		if scanErr := rows.Scan(&userID, &lastUsedRaw); scanErr != nil {
			return nil, scanErr
		}
		lastUsedAt, ok := parseSQLTime(lastUsedRaw, time.UTC)
		if !ok {
			return nil, fmt.Errorf("parse latest usage time for user %d: unsupported database value %T (%v)", userID, lastUsedRaw, lastUsedRaw)
		}
		ts := lastUsedAt.UTC()
		result[userID] = &ts
	}
