...
func incrementUsageBillingSubscription(ctx context.Context, tx *sql.Tx, subscriptionID int64, costUSD float64) error {
	...
	`
	if isSQLiteExecutor(ctx, tx) {
		updateSQL = `
			UPDATE user_subscriptions
			SET daily_usage_usd = daily_usage_usd + ?,
				weekly_usage_usd = weekly_usage_usd + ?,
			...
			WHERE id = ?
				...
				AND EXISTS (
					...
					WHERE g.id = user_subscriptions.group_id AND g.deleted_at IS NULL
				)
		`
	}
...
func incrementUsageBillingCarpoolFiveHour(ctx context.Context, tx *sql.Tx, groupID, userID int64, costUSD float64, occurredAt time.Time) error {
	...
	if isSQLiteExecutor(ctx, tx) {
		updateSQL = `
			UPDATE carpool_members AS m
			SET five_hour_used_usd = CASE
					WHEN m.five_hour_window_start IS NULL OR datetime(m.five_hour_window_start, '+5 hours') <= ? THEN ?
					ELSE COALESCE(m.five_hour_used_usd, 0) + ?
				END,
				five_hour_window_start = CASE
					WHEN m.five_hour_window_start IS NULL OR datetime(m.five_hour_window_start, '+5 hours') <= ? THEN ?
			...
			WHERE m.user_id = ?
				...
				AND m.status = 'active'
				AND EXISTS (
					SELECT 1 FROM carpool_pools p
					WHERE p.id = m.pool_id AND p.group_id = ? AND p.deleted_at IS NULL
				)
		`
		_, err := tx.ExecContext(ctx, updateSQL, occurredAt, costUSD, costUSD, occurredAt, occurredAt, userID, groupID)
		return err
	}
	_, err := tx.ExecContext(ctx, updateSQL, occurredAt, costUSD, costUSD, occurredAt, occurredAt, groupID, userID)
	return err
}
