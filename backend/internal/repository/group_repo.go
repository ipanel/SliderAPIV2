...
func isSQLiteSQLExecutor(ctx context.Context, exec sqlExecutor) bool {
	if queryer, ok := exec.(sqlRowQuerier); ok {
		return isSQLiteExecutor(ctx, queryer)
	}

	var version string
	return scanSingleRow(ctx, exec, "SELECT sqlite_version()", nil, &version) == nil && version != ""
}
...
func (r *groupRepository) BindAccountsToGroup(ctx context.Context, groupID int64, accountIDs []int64) error {
	...
	query := `INSERT IGNORE INTO account_groups (account_id, group_id, priority, created_at)
		 SELECT gid, ?, 50, NOW() FROM JSON_TABLE(?, '$[*]' COLUMNS (gid BIGINT PATH '$')) jt;`
	if isSQLiteSQLExecutor(ctx, r.sql) {
		query = `INSERT INTO account_groups (account_id, group_id, priority, created_at)
			 SELECT CAST(j.value AS INTEGER), ?, 50, CURRENT_TIMESTAMP
			 FROM json_each(CAST(? AS TEXT)) AS j
			 WHERE 1
			 ON CONFLICT(account_id, group_id) DO NOTHING;`
	}
	_, err := r.sql.ExecContext(ctx, query, groupID, JSONInt64Slice(accountIDs))
	if err != nil {
		return err
	}
