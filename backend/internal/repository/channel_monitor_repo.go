...
func (r *channelMonitorRepository) ListRecentHistoryForMonitors(
	...
	if isSQLiteExecutor(ctx, r.db) {
		query = `
			WITH targets AS (
			    SELECT CAST(json_extract(j.value, '$[0]') AS INTEGER) AS monitor_id,
			           CAST(json_extract(j.value, '$[1]') AS TEXT) AS model
	...
	rows, err := r.db.QueryContext(ctx, query, mustInt64StringPairsValue(pairIDs, pairModels), perMonitorLimit)
	if err != nil {
