...
func TestValidateMigrationExecutionMode(t *testing.T) {
	...
	t.Run("notx迁移要求CREATE使用IF NOT EXISTS", func(t *testing.T) {
		nonTx, err := validateMigrationExecutionMode("001_add_idx_notx.sql", "CREATE INDEX  idx_a ON t(a);")
		require.False(t, nonTx)
		require.Error(t, err)
	})

	t.Run("notx迁移要求DROP使用IF EXISTS", func(t *testing.T) {
		nonTx, err := validateMigrationExecutionMode("001_drop_idx_notx.sql", "DROP INDEX  idx_a;")
		require.False(t, nonTx)
		require.Error(t, err)
	})

	t.Run("notx迁移禁止事务控制语句", func(t *testing.T) {
		nonTx, err := validateMigrationExecutionMode("001_add_idx_notx.sql", "BEGIN; CREATE INDEX  IF NOT EXISTS idx_a ON t(a); COMMIT;")
		require.False(t, nonTx)
		require.Error(t, err)
	})

	t.Run("notx迁移禁止混用非CONCURRENTLY语句", func(t *testing.T) {
		nonTx, err := validateMigrationExecutionMode("001_add_idx_notx.sql", "CREATE INDEX  IF NOT EXISTS idx_a ON t(a); UPDATE t SET a = 1;")
		require.False(t, nonTx)
		require.Error(t, err)
	})
...
func TestApplyMigrationsFS_NonTransactionalMigration(t *testing.T) {
	...
	mock.ExpectExec("CREATE INDEX  IF NOT EXISTS idx_t_a ON t\\(a\\)").
	...
	fsys := fstest.MapFS{
		"001_add_idx_notx.sql": &fstest.MapFile{
			Data: []byte("CREATE INDEX  IF NOT EXISTS idx_t_a ON t(a);"),
		},
	}
...
func TestApplyMigrationsFS_NonTransactionalMigration_MultiStatements(t *testing.T) {
	...
	mock.ExpectExec("CREATE INDEX  IF NOT EXISTS idx_t_a ON t\\(a\\)").
		WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("CREATE INDEX  IF NOT EXISTS idx_t_b ON t\\(b\\)").
		WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("INSERT INTO schema_migrations \\(filename, checksum\\) VALUES \\(\\?, \\?\\)").
...
-- second
CREATE INDEX  IF NOT EXISTS idx_t_b ON t(b);
`),
		},
	}
...
func TestApplyMigrationsFS_OwnedAccountIdentityUniqueMigration_FailsFastOnDuplicatePrecheck(t *testing.T) {
	...
	fsys := fstest.MapFS{
		"140_owned_account_identity_unique_notx.sql": &fstest.MapFile{
			Data: []byte("CREATE UNIQUE INDEX  IF NOT EXISTS idx_accounts_owned_openai_chatgpt_account_id_uniq ON accounts (owner_user_id, NULLIF(TRIM(JSON_UNQUOTE(JSON_EXTRACT(credentials, '$.chatgpt_account_id'))), '')) WHERE deleted_at IS NULL;"),
		},
	}
...
func TestApplyMigrationsFS_OwnedAccountIdentityUniqueMigration_DropsInvalidIndexesBeforeRetry(t *testing.T) {
	...
	mock.ExpectExec("CREATE UNIQUE INDEX  IF NOT EXISTS idx_accounts_owned_openai_chatgpt_account_id_uniq").
		WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("INSERT INTO schema_migrations \\(filename, checksum\\) VALUES \\(\\?, \\?\\)").
