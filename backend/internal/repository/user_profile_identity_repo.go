...
func lockRepositoryScopedKeys(ctx context.Context, client *dbent.Client, exec sqlQueryExecutor, keys ...string) (func(), error) {
	...
	db := drv.DB()
	for _, key := range normalized {
		dbRelease, err := dblock.AcquireNamedLock(
			ctx,
			db,
			fmt.Sprintf("ikik_api_profile_identity_%d", advisoryLockHash(key)),
			30,
		)
		if err != nil {
			release()
...
func (r *userRepository) RecordProviderGrant(ctx context.Context, input ProviderGrantRecordInput) (bool, error) {
	...
	result, err := exec.ExecContext(ctx, providerDefaultGrantInsertSQL(dialectName),
		input.UserID,
...
func providerDefaultGrantInsertSQL(dialectName string) string {
	switch dialectName {
	case dialect.MySQL:
		return `INSERT IGNORE INTO user_provider_default_grants (user_id, provider_type, grant_reason) VALUES (?, ?, ?);`
	case dialect.SQLite:
		return `INSERT OR IGNORE INTO user_provider_default_grants (user_id, provider_type, grant_reason) VALUES (?, ?, ?);`
	default:
		return `INSERT INTO user_provider_default_grants (user_id, provider_type, grant_reason) VALUES (?, ?, ?);`
	}
}
...
func (r *userRepository) UpsertUserAvatar(ctx context.Context, userID int64, input service.UpsertUserAvatarInput) (*service.UserAvatar, error) {
	...
	_, err = exec.ExecContext(ctx, upsertUserAvatarSQL(dialectName),
		userID,
...
func upsertUserAvatarSQL(dialectName string) string {
...
ON CONFLICT (user_id) DO UPDATE SET storage_provider = excluded.storage_provider,
