...
func TestUsageCleanupRepositoryClaimNextPendingTaskNone(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id FROM usage_cleanup_tasks").
...
func TestUsageCleanupRepositoryClaimNextPendingTask(t *testing.T) {
	...
	mock.ExpectExec("UPDATE usage_cleanup_tasks").
		WithArgs(service.UsageCleanupStatusRunning, int64(4), service.UsageCleanupStatusPending, service.UsageCleanupStatusRunning, int64(1800)).
...
func TestUsageCleanupRepositoryExportUsageLogs(t *testing.T) {
	...
	mock.ExpectQuery("SELECT JSON_OBJECT\\(").
		WithArgs(start, end).
...
func TestUsageCleanupRepositorySnapshotUsageLogs(t *testing.T) {
	...
	mock.ExpectExec("CONVERT_TZ\\(created_at, @@session.time_zone, 'Asia/Shanghai'\\)").
		WithArgs(start, end).
		WillReturnResult(sqlmock.NewResult(0, 2))
	mock.ExpectExec("CONVERT_TZ\\(ul.created_at, @@session.time_zone, 'Asia/Shanghai'\\)").
		WithArgs(start, end).
		WillReturnResult(sqlmock.NewResult(0, 2))
...
func TestUsageCleanupRepositoryFindOldestUsageLogBefore(t *testing.T) {
	...
	mock.ExpectQuery("SELECT MIN\\(created_at\\) FROM usage_logs WHERE created_at < \\?").
		WithArgs(cutoff).
...
func TestBuildUsageCleanupWhereRequestTypePriority(t *testing.T) {
	...
	require.Equal(t, "created_at >= ? AND created_at <= ? AND (request_type = ? OR (request_type = ? AND openai_ws_mode = TRUE))", where)
	require.Equal(t, []any{start, end, requestType, int16(0)}, args)
}
...
func TestBuildUsageCleanupWhereRequestTypeLegacyFallback(t *testing.T) {
	...
	require.Equal(t, "created_at >= ? AND created_at <= ? AND (request_type = ? OR (request_type = ? AND stream = TRUE AND openai_ws_mode = FALSE))", where)
	require.Equal(t, []any{start, end, requestType, int16(0)}, args)
}
