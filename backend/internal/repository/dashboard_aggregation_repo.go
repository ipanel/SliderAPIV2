	...
	query := `
		...
		FROM (
			SELECT
				...
				ROUND(COALESCE(SUM(total_cost), 0), 10) AS total_cost,
				ROUND(COALESCE(SUM(actual_cost), 0), 10) AS actual_cost,
				ROUND(COALESCE(SUM(COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1)), 0), 10) AS account_cost,
	...
	query := `
		...
		FROM (
			SELECT
				...
				ROUND(COALESCE(SUM(total_cost), 0), 10) AS total_cost,
				ROUND(COALESCE(SUM(actual_cost), 0), 10) AS actual_cost,
				ROUND(COALESCE(SUM(account_cost), 0), 10) AS account_cost,
