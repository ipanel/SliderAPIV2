...
func (r *opsRepository) GetThroughputTrend(ctx context.Context, filter *service.OpsDashboardFilter, bucketSeconds int) (*service.OpsThroughputTrendResponse, error) {
	...
	switchEventExpr := "ev"
	switchEventJoin := `CROSS JOIN JSON_TABLE(
    COALESCE(NULLIF(upstream_errors, 'null'), '[]'),
    '$[*]' COLUMNS (ev JSON PATH '$')
  ) AS ev`
	if isSQLiteExecutor(ctx, r.db) {
		switchEventExpr = "ev.value"
		switchEventJoin = `CROSS JOIN json_each(CAST(COALESCE(NULLIF(upstream_errors, 'null'), '[]') AS TEXT)) AS ev`
	}
...
switch_buckets AS (
  SELECT ` + errorBucketExpr + ` AS bucket,
         COALESCE(SUM(CASE
           WHEN SUBSTRING_INDEX(JSON_UNQUOTE(JSON_EXTRACT(` + switchEventExpr + `, '$.kind')), ':', 1) IN ('failover', 'retry_exhausted_failover', 'failover_on_400') THEN 1
           ELSE 0
