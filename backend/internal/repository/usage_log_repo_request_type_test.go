...
func TestUsageLogRepositoryListWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("SELECT COUNT\\(\\*\\) FROM usage_logs WHERE \\(request_type = \\? OR \\(request_type = \\? AND openai_ws_mode = TRUE\\)\\)").
		WithArgs(requestType, int16(service.RequestTypeUnknown)).
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(int64(0)))
	mock.ExpectQuery("SELECT .* FROM usage_logs WHERE \\(request_type = \\? OR \\(request_type = \\? AND openai_ws_mode = TRUE\\)\\) ORDER BY id DESC LIMIT \\? OFFSET \\?").
		WithArgs(requestType, int16(service.RequestTypeUnknown), 20, 0).
...
func TestUsageLogRepositoryGetUsageTrendWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("AND \\(request_type = \\? OR \\(request_type = \\? AND stream = TRUE AND openai_ws_mode = FALSE\\)\\)").
		WithArgs(start, end, requestType, int16(service.RequestTypeUnknown)).
...
func TestUsageLogRepositoryGetModelStatsWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("AND \\(request_type = \\? OR \\(request_type = \\? AND openai_ws_mode = TRUE\\)\\)").
		WithArgs(start, end, requestType, int16(service.RequestTypeUnknown)).
...
func TestUsageLogRepositoryGetStatsWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("FROM usage_logs\\s+WHERE \\(request_type = \\? OR \\(request_type = \\? AND stream = FALSE AND openai_ws_mode = FALSE\\)\\)").
		WithArgs(requestType, int16(service.RequestTypeUnknown)).
	...
	mock.ExpectQuery("SELECT COALESCE\\(NULLIF\\(TRIM\\(inbound_endpoint\\), ''\\), 'unknown'\\) AS endpoint").
		WithArgs(sqlmock.AnyArg(), sqlmock.AnyArg(), requestType, int16(service.RequestTypeUnknown)).
...
func TestBuildRequestTypeFilterConditionLegacyFallback(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantWhere: "(request_type = ? OR (request_type = ? AND stream = FALSE AND openai_ws_mode = FALSE))",
			wantArg:   int16(service.RequestTypeSync),
		},
		{
			name:      "stream_with_legacy_fallback",
			request:   int16(service.RequestTypeStream),
			wantWhere: "(request_type = ? OR (request_type = ? AND stream = TRUE AND openai_ws_mode = FALSE))",
			wantArg:   int16(service.RequestTypeStream),
		},
		{
			name:      "ws_v2_with_legacy_fallback",
			request:   int16(service.RequestTypeWSV2),
			wantWhere: "(request_type = ? OR (request_type = ? AND openai_ws_mode = TRUE))",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.request == int16(service.RequestTypeUnknown) || tt.request == 99 {
				require.Equal(t, []any{tt.wantArg}, args)
			} else {
				require.Equal(t, []any{tt.wantArg, int16(service.RequestTypeUnknown)}, args)
			}
		})
	}
}
...
func TestBuildRequestTypeFilterConditionWithAlias(t *testing.T) {
	...
	require.Equal(t, "(ul.request_type = ? OR (ul.request_type = ? AND ul.stream = TRUE AND ul.openai_ws_mode = FALSE))", where)
	require.Equal(t, []any{int16(service.RequestTypeStream), int16(service.RequestTypeUnknown)}, args)
}
