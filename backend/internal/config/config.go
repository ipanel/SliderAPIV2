...
func (d *DatabaseConfig) IsSQLite() bool {
	return d.DriverName() == DatabaseDriverSQLite
}
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	// ResolveDataDir centralizes explicit, portable, Docker, and legacy config
	// locations so setup detection and configuration loading cannot diverge.
	viper.AddConfigPath(ResolveDataDir())
...
func setDefaults() {
	...
	// Database
	viper.SetDefault("database.driver", DatabaseDriverMySQL)
	viper.SetDefault("database.path", "ikik-api.db")
...
func (c *Config) Validate() error {
	...
	default:
		return fmt.Errorf("database.driver must be one of: mysql/sqlite")
	}
...
func GetServerAddress() string {
	...
	v.AddConfigPath(ResolveDataDir())
