...
func TestDatabaseConfigSQLiteDSN(t *testing.T) {
	d := &DatabaseConfig{Driver: DatabaseDriverSQLite, Path: `C:\data\ikik.db`}
	got := d.DSN()
	if !strings.HasPrefix(got, "file:C:/data/ikik.db?") {
	...
	if !strings.Contains(got, "_pragma=foreign_keys(1)") || !strings.Contains(got, "_pragma=busy_timeout(5000)") {
		t.Fatalf("missing SQLite pragmas: %q", got)
	}
}

func TestDatabaseDriverDefaultsToMySQL(t *testing.T) {
	if got := (&DatabaseConfig{}).DriverName(); got != DatabaseDriverMySQL {
