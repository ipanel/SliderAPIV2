...
func EffectiveOAuthConfig(cfg OAuthConfig, oauthType string) (OAuthConfig, error) {
	...
	// Fall back to the deployment-provided Gemini CLI OAuth client when not configured.
	...
	if effective.ClientID == "" && effective.ClientSecret == "" {
		...
		effective.ClientSecret = builtinClientSecret
		isBuiltinClient = true
	} else if effective.ClientID == "" || effective.ClientSecret == "" {
		return OAuthConfig{}, infraerrors.New(http.StatusBadRequest, "GEMINI_OAUTH_CLIENT_NOT_CONFIGURED", "OAuth client not configured: please set both client_id and client_secret (or leave both empty to use environment variables)")
	}
