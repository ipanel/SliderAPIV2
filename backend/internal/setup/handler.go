...
func testDatabase(c *gin.Context) {
	...
	cfg := &DatabaseConfig{Driver: req.Driver, Path: req.Path, Host: strings.TrimSpace(req.Host), Port: req.Port, User: strings.TrimSpace(req.User), Password: req.Password, DBName: strings.TrimSpace(req.DBName), SSLMode: strings.TrimSpace(req.SSLMode)}
...
func install(c *gin.Context) {
	...
	if err := validateDatabaseConfig(&req.Database); err != nil {
		response.Error(c, http.StatusBadRequest, err.Error())
		return
	}
