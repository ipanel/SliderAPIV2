...
type BackupRecord struct {
	...
	BackupType    string `json:"backup_type"` // mysql, sqlite
...
// databaseBackupProfile describes the backup artifact produced by the active database driver.
type databaseBackupProfile struct {
	backupType    string
...
func (s *BackupService) backupProfile() databaseBackupProfile {
	if s.dbCfg.DriverName() == config.DatabaseDriverSQLite {
		return databaseBackupProfile{
			backupType:    config.DatabaseDriverSQLite,
			fileExtension: ".sqlite.gz",
			dumpOperation: "sqlite backup",
			restoreAction: "sqlite restore",
		}
	}

	return databaseBackupProfile{
		backupType:    config.DatabaseDriverMySQL,
		fileExtension: ".sql.gz",
		dumpOperation: "mariadb-dump",
		restoreAction: "mariadb restore",
	}
}

func (s *BackupService) backupFileName(now time.Time, profile databaseBackupProfile) string {
	databaseName := strings.TrimSpace(s.dbCfg.DBName)
	if databaseName == "" {
		databaseName = "database"
	}
	return fmt.Sprintf("%s_%s%s", databaseName, now.Format("20060102_150405"), profile.fileExtension)
}

func (s *BackupService) validateRestoreBackupType(record *BackupRecord) error {
	profile := s.backupProfile()
	backupType := strings.TrimSpace(record.BackupType)
	if profile.backupType == config.DatabaseDriverMySQL && backupType == "" {
		// Legacy MySQL records did not always persist a backup type.
		return nil
	}
	if backupType != profile.backupType {
		return infraerrors.BadRequest(
			"BACKUP_TYPE_NOT_RESTORABLE",
			fmt.Sprintf("backup type %q cannot be restored to %s database", backupType, profile.backupType),
		)
	}
	return nil
}
...
func (s *BackupService) CreateBackup(ctx context.Context, triggeredBy string, expireDays int) (*BackupRecord, error) {
	...
	profile := s.backupProfile()
	fileName := s.backupFileName(now, profile)
	...
	// Stream the driver-specific dump through gzip to object storage.
	dumpReader, err := s.dumper.Dump(ctx)
	if err != nil {
		record.Status = "failed"
		record.ErrorMsg = fmt.Sprintf("%s failed: %v", profile.dumpOperation, err)
		record.FinishedAt = time.Now().Format(time.RFC3339)
		_ = s.saveRecord(ctx, record)
		return record, fmt.Errorf("%s: %w", profile.dumpOperation, err)
	}
...
func (s *BackupService) executeBackup(record *BackupRecord, objectStore BackupObjectStore) {
	...
	// Stage 1: driver-specific database dump.
...
func (s *BackupService) RestoreBackup(ctx context.Context, backupID string) error {
	...
	if err := s.validateRestoreBackupType(record); err != nil {
		return err
	}
	...
	if err := s.dumper.Restore(ctx, gzReader); err != nil {
		return fmt.Errorf("%s: %w", s.backupProfile().restoreAction, err)
	}
...
func (s *BackupService) executeRestore(record *BackupRecord, objectStore BackupObjectStore) {
	...
	if err := s.dumper.Restore(ctx, gzReader); err != nil {
		record.RestoreStatus = "failed"
		record.RestoreError = fmt.Sprintf("%s: %v", s.backupProfile().restoreAction, err)
		_ = s.saveRecord(context.Background(), record)
		return
	}
