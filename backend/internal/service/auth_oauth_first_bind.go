...
func (s *AuthService) applyProviderDefaultSettingsOnFirstBind(
	...
	insertSQL := `INSERT INTO user_provider_default_grants (user_id, provider_type, grant_reason) VALUES (?, ?, ?);`
	switch client.Driver().Dialect() {
	case dialect.MySQL:
		insertSQL = `INSERT IGNORE INTO user_provider_default_grants (user_id, provider_type, grant_reason) VALUES (?, ?, ?);`
	case dialect.SQLite:
		insertSQL = `INSERT OR IGNORE INTO user_provider_default_grants (user_id, provider_type, grant_reason) VALUES (?, ?, ?);`
	}
