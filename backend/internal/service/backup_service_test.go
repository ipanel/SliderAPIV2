...
func TestBackupService_CreateBackup_Streaming(t *testing.T) {
	...
	require.Equal(t, config.DatabaseDriverMySQL, record.BackupType)
	require.True(t, strings.HasSuffix(record.FileName, ".sql.gz"), record.FileName)
...
func TestBackupService_CreateBackup_DumpFailure(t *testing.T) {
	...
	dumper := &mockDumper{dumpErr: fmt.Errorf("boom")}
	store := newMockObjectStore()
	svc := newTestBackupService(repo, dumper, store)

	record, err := svc.CreateBackup(context.Background(), "manual", 14)
	require.Error(t, err)
	require.Contains(t, err.Error(), "mariadb-dump")
	require.Equal(t, "failed", record.Status)
	require.Contains(t, record.ErrorMsg, "mariadb-dump failed")
}
...
func TestBackupService_CreateBackup_SQLiteMetadataAndErrorLabel(t *testing.T) {
	t.Run("completed backup", func(t *testing.T) {
		repo := newMockSettingRepo()
		seedS3Config(t, repo)
		dumper := &mockDumper{dumpData: []byte("sqlite database bytes")}
		svc := newTestBackupService(repo, dumper, newMockObjectStore())
		svc.dbCfg.Driver = config.DatabaseDriverSQLite
		svc.dbCfg.DBName = ""

		record, err := svc.CreateBackup(context.Background(), "manual", 14)
		require.NoError(t, err)
		require.Equal(t, config.DatabaseDriverSQLite, record.BackupType)
		require.True(t, strings.HasPrefix(record.FileName, "database_"), record.FileName)
		require.True(t, strings.HasSuffix(record.FileName, ".sqlite.gz"), record.FileName)
		require.True(t, strings.HasSuffix(record.S3Key, ".sqlite.gz"), record.S3Key)
	})

	t.Run("dump failure", func(t *testing.T) {
		repo := newMockSettingRepo()
		seedS3Config(t, repo)
		svc := newTestBackupService(repo, &mockDumper{dumpErr: fmt.Errorf("boom")}, newMockObjectStore())
		svc.dbCfg.Driver = config.DatabaseDriverSQLite

		record, err := svc.CreateBackup(context.Background(), "manual", 14)
		require.Error(t, err)
		require.Contains(t, err.Error(), "sqlite backup")
		require.Equal(t, config.DatabaseDriverSQLite, record.BackupType)
		require.Contains(t, record.ErrorMsg, "sqlite backup failed")
		require.NotContains(t, record.ErrorMsg, "mariadb-dump")
	})
}

func TestBackupService_StartBackup_SQLiteMetadata(t *testing.T) {
	repo := newMockSettingRepo()
	seedS3Config(t, repo)
	dumper := &blockingDumper{blockCh: make(chan struct{}), data: []byte("sqlite database bytes")}
	svc := newTestBackupService(repo, dumper, newMockObjectStore())
	svc.dbCfg.Driver = config.DatabaseDriverSQLite
	...
	require.NoError(t, err)
	require.Equal(t, "completed", final.Status)
	require.Equal(t, config.DatabaseDriverSQLite, final.BackupType)
}

func TestBackupService_RestoreBackupTypeMatchesDatabaseDriver(t *testing.T) {
	tests := []struct {
		name          string
		driver        string
		backupType    string
		...
		{name: "mysql", driver: config.DatabaseDriverMySQL, backupType: config.DatabaseDriverMySQL, shouldRestore: true},
		{name: "mysql legacy empty type", driver: config.DatabaseDriverMySQL, backupType: "", shouldRestore: true},
		{name: "sqlite", driver: config.DatabaseDriverSQLite, backupType: config.DatabaseDriverSQLite, shouldRestore: true},
		{name: "mysql rejects sqlite", driver: config.DatabaseDriverMySQL, backupType: config.DatabaseDriverSQLite},
		{name: "sqlite rejects mysql", driver: config.DatabaseDriverSQLite, backupType: config.DatabaseDriverMySQL},
		{name: "sqlite rejects legacy empty type", driver: config.DatabaseDriverSQLite, backupType: ""},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo := newMockSettingRepo()
			seedS3Config(t, repo)
			dumper := &mockDumper{dumpData: []byte("database backup")}
			store := newMockObjectStore()
			svc := newTestBackupService(repo, dumper, store)
			svc.dbCfg.Driver = tt.driver

			record, err := svc.CreateBackup(context.Background(), "manual", 14)
			require.NoError(t, err)
			record.BackupType = tt.backupType
			require.NoError(t, svc.saveRecord(context.Background(), record))

			err = svc.RestoreBackup(context.Background(), record.ID)
			if tt.shouldRestore {
				require.NoError(t, err)
				require.Equal(t, []byte("database backup"), dumper.restored)
				return
			}
			require.Error(t, err)
			require.Equal(t, "BACKUP_TYPE_NOT_RESTORABLE", infraerrors.Reason(err))
			require.Contains(t, err.Error(), tt.driver)
		})
	}
}

func TestBackupService_StartRestoreRejectsMismatchedDatabaseType(t *testing.T) {
	tests := []struct {
		name       string
		driver     string
		backupType string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := svc.StartRestore(context.Background(), "mismatched")
			require.Error(t, err)
			require.Equal(t, "BACKUP_TYPE_NOT_RESTORABLE", infraerrors.Reason(err))
		})
	}
...
func TestBackupService_RestoreErrorLabelMatchesDatabaseDriver(t *testing.T) {
	tests := []struct {
		name      string
		driver    string
		wantLabel string
	}{
		{name: "mysql", driver: config.DatabaseDriverMySQL, wantLabel: "mariadb restore"},
		{name: "sqlite", driver: config.DatabaseDriverSQLite, wantLabel: "sqlite restore"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo := newMockSettingRepo()
			seedS3Config(t, repo)
			dumper := &mockDumper{dumpData: []byte("database backup"), restErr: fmt.Errorf("boom")}
			svc := newTestBackupService(repo, dumper, newMockObjectStore())
			svc.dbCfg.Driver = tt.driver
			...
			err = svc.RestoreBackup(context.Background(), record.ID)
			require.Error(t, err)
			require.Contains(t, err.Error(), tt.wantLabel)
		})
	}
}

func TestBackupService_StartRestore_SQLiteErrorLabel(t *testing.T) {
	repo := newMockSettingRepo()
	seedS3Config(t, repo)
	dumper := &mockDumper{dumpData: []byte("database backup"), restErr: fmt.Errorf("boom")}
	svc := newTestBackupService(repo, dumper, newMockObjectStore())
	...
	require.Contains(t, final.RestoreError, "sqlite restore")
	require.NotContains(t, final.RestoreError, "mariadb restore")
}
