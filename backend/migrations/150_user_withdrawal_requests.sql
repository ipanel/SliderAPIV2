...
ALTER TABLE user_withdrawal_requests
    ADD COLUMN IF NOT EXISTS pending_user_id_key BIGINT;

-- MariaDB cannot index generated columns that reference foreign-key columns;
-- one-pending-per-user is enforced by the application layer.
CREATE INDEX IF NOT EXISTS user_withdrawal_requests_one_pending_per_user
    ON user_withdrawal_requests (pending_user_id_key);
