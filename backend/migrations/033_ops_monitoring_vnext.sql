...
-- (migrated from COMMENT ON TABLE) ops_error_logs
...
-- (migrated from COMMENT ON TABLE) ops_retry_attempts
...
-- (migrated from COMMENT ON TABLE) ops_system_metrics
...
-- (migrated from COMMENT ON TABLE) ops_job_heartbeats
...
ALTER TABLE ops_metrics_hourly
    ADD COLUMN IF NOT EXISTS platform_dim VARCHAR(32) AS (COALESCE(platform, '')) STORED,
    ADD COLUMN IF NOT EXISTS group_id_dim BIGINT AS (COALESCE(group_id, 0)) STORED;

CREATE UNIQUE INDEX IF NOT EXISTS idx_ops_metrics_hourly_unique_dim
    ON ops_metrics_hourly (bucket_start, platform_dim, group_id_dim);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_ops_metrics_daily_unique_dim
    ON ops_metrics_daily (bucket_date, platform_dim, group_id_dim);
...
ALTER TABLE ops_retry_attempts
    ADD COLUMN IF NOT EXISTS source_error_id_active BIGINT AS (
        CASE WHEN source_error_id IS NOT NULL AND status IN ('queued', 'running')
             THEN source_error_id ELSE NULL END
    ) STORED;

CREATE UNIQUE INDEX IF NOT EXISTS idx_ops_retry_attempts_unique_active
    ON ops_retry_attempts (source_error_id_active);

-- ============================================
-- 2) Optional: pg_trgm + trigram indexes for fuzzy search
-- ============================================

-- MariaDB has no pg_trgm/GIN; keep plain btree indexes for these columns.
...
-- Hourly table
ALTER TABLE ops_metrics_hourly
    ADD COLUMN IF NOT EXISTS duration_avg_ms DOUBLE,
    ADD COLUMN IF NOT EXISTS duration_max_ms INT,
    ADD COLUMN IF NOT EXISTS ttft_avg_ms DOUBLE,
...
-- (migrated from COMMENT ON COLUMN) ops_system_metrics.redis_conn_total
-- (migrated from COMMENT ON COLUMN) ops_system_metrics.redis_conn_idle