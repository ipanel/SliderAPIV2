...
-- Backfill from the legacy users.allowed_groups JSON array (MariaDB rewrite of LATERAL unnest).
...
JOIN groups g ON g.id = CAST(JSON_EXTRACT(u.allowed_groups, CONCAT('$[', n.idx, ']')) AS SIGNED)
